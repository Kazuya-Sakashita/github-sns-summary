...
// ✅ Recharts の内部型に依存しない “必要最小” の payload 型
type TooltipItem = {
  name?: string | number
...
function isFormatter(fn: unknown): fn is TooltipFormatter {
  return typeof fn === "function"
}

function ChartTooltipContent({
  active,
  payload, // ← Recharts 側の payload 型（labelFormatter の第2引数にそのまま渡せる）
  className,
  indicator = "dot",
  hideLabel = false,
  hideIndicator = false,
  ...
  // ✅ UI描画用には “必要最小” の型で扱う
  const safePayload = React.useMemo(() => (payload ?? []) as unknown as TooltipItem[], [payload])
  const hasPayload = safePayload.length > 0
  ...
  const tooltipLabel = React.useMemo(() => {
    ...
    if (labelFormatter) {
      // ✅ Recharts が期待する “本来の payload” を第2引数に渡す（TS2554 対策）
      return (
      ...
      <div className="grid gap-1.5">
        {safePayload.map((item, index) => {
          ...
          const indicatorColor =
            color ||
            (typeof payloadObj?.fill === "string" ? payloadObj.fill : undefined) ||
            item.color
          ...
          return (
            ...
            >
              {isFormatter(formatter) && item.value !== undefined && item.name ? (
